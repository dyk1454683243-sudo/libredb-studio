...
describe("OverviewTab", () => {
  ...
  test("maps audit events into the activity feed and formats their relative times", async () => {
    ...
    await waitFor(() => {
      ...
      // #992: the Audit affordance is not gated on an empty preview
  ...
  // #992: this feed reads the same per-process buffer #851 showed is incomplete.
  // The Audit affordance is unconditional so the #989 disclosure is one click
  // away whether the preview is empty or populated, and the empty copy must not
  // read as "no security events occurred".
  test("Recent Activity always links to Audit and does not claim an empty feed is complete", async () => {
    mockGetHistory.mockImplementation(() => []);
